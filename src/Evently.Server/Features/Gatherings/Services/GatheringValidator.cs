...
public sealed class GatheringValidator : AbstractValidator<Gathering>
{
	public GatheringValidator()
	{
		...
		RuleFor((exhibition) => exhibition.Description)
			.NotEmpty()
			.WithMessage("Description is required.");
		RuleFor((exhibition) => exhibition.Start)
			.NotEmpty()
			.WithMessage("Starting Date is required.");
		RuleFor((exhibition) => exhibition.End).NotEmpty().WithMessage("End Date is required.");
		RuleFor((exhibition) => exhibition.OrganiserId)
			.NotEmpty()
			.WithMessage("Event Organiser Id is required.");
		RuleForEach((exhibition) => exhibition.Bookings)
			.Custom(
				(value, context) =>
				{
