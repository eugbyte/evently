...
) : ControllerBase
{
	...
	public async Task<ActionResult<Gathering>> CreateGathering(
		[FromForm] GatheringReqDto gatheringReqDto,
	...
	public async Task<ActionResult> UpdateGathering(
		long gatheringId,
		[FromForm] GatheringReqDto gatheringReqDto,
		[FromForm] IFormFile? coverImg
	)
	...
	{
		string fileName =
			$"gatherings/{gatheringId}/cover-image{Path.GetExtension(coverImg.FileName)}";
		...
		Uri uri = await objectStorageService.UploadFile(
			_containerName,
			fileName,
			binaryData,
